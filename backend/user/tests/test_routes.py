...
class TestCreate:
    ...
    async def test_ok(self, url: str, client: AsyncClient):
        response = await client.post(url, json={"tg_id": 1337, "username": "abobert"})

        assert response.status_code == status.HTTP_201_CREATED
        assert (await select(User).filter(User.tg_id == 1337).exists()).scalar() is True
...
class TestProfile:
    ...
    async def test_ok(self, get_url, user_client: AsyncClient):
        user = await UserFactory(tg_id=123)

        response = await user_client.get(get_url(123))
