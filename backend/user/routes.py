...
from user.schemas import UserCreate, UserFull
...
async def create_user(user_create: UserCreate, session: AsyncSession = Depends(get_session)):
    return await User.create(tg_id=user_create.tg_id, username=user_create.username, session=session)


@router.get("/{tg_id}/", response_model=UserFull)
async def get_user_profile(tg_id: int, session: AsyncSession = Depends(get_session)):
    return await User.get_by_tg_id(tg_id, session=session)
